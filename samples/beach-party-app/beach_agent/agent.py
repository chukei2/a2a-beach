...
class BeachAgent:  # Renamed from BeachAgent
    ...
    def __init__(self, mcp_tools: List[Any]):  # Modified to accept mcp_tools
        """
        Initializes the BeachAgent. # Updated docstring

        Args:
            mcp_tools: A list of preloaded MCP (Multi-Process Controller) tools.
        """
        logger.info("Initializing BeachAgent with preloaded MCP tools...") # Updated log
        try:
        ...
        if not self.mcp_tools:
            logger.warning(
                "BeachAgent initialized with no MCP tools. Beach search functionality may be limited." # Updated log
            )
        else:
            logger.info(
                f"BeachAgent initialized with {len(self.mcp_tools)} MCP tools." # Updated log
            )

    async def ainvoke(self, query: str, sessionId: str) -> dict[str, Any]:
        logger.info(
            f"BeachAgent.ainvoke called with query: '{query}', sessionId: '{sessionId}'" # Updated log
        )
        ...
        try:
            ...
            if not self.mcp_tools:
                logger.error(
                    "No MCP tools available for BeachAgent.ainvoke. Cannot perform beach search." # Updated log
                )
                return {
                    "is_task_complete": True,  # Or False if you want to signal an error state differently
                    "require_user_input": False,
                    "content": "I'm sorry, but the beach search tool is currently unavailable. Please try again later.", # Updated message
                }
            logger.debug(
                f"Using preloaded MCP Tools for Beach task: {len(self.mcp_tools)} tools." # Updated log
            )

            beach_agent_runnable = create_react_agent( # Renamed variable
                self.model,
            ...
            logger.debug(
                "LangGraph React agent for Beach task created/configured with preloaded tools." # Updated log
            )
            ...
            logger.debug(
                f"Invoking Beach agent with input: {langgraph_input} and config: {config}" # Updated log
            )

            await beach_agent_runnable.ainvoke(langgraph_input, config) # Using renamed variable
            logger.debug(
                "Beach agent ainvoke call completed. Fetching response from state..." # Updated log
            )

            response = self._get_agent_response_from_state(
                config, beach_agent_runnable # Using renamed variable
            )
            logger.info(
                f"Response from Beach agent state for session {sessionId}: {response}" # Updated log
            )
            return response

        except httpx.HTTPStatusError as http_err:
            logger.error(
                f"HTTPStatusError in BeachAgent.ainvoke (Beach task): {http_err.response.status_code} - {http_err}", # Updated log
                exc_info=True,
            )
            return {
                "is_task_complete": True,
                "require_user_input": False,
                "content": f"An error occurred with an external service for Beach task: {http_err.response.status_code}", # Updated log
            }
        except Exception as e:
            logger.error(
                f"Unhandled exception in BeachAgent.ainvoke (Beach task): {type(e).__name__} - {e}", # Updated log
                exc_info=True,
            )
            ...
            return {
                ...
                "require_user_input": False,
                "content": f"An unexpected error occurred while processing your beach request: {type(e).__name__}.", # Updated message
            }
    ...
    async def stream(self, query: str, sessionId: str) -> AsyncIterable[Any]:
        logger.info(
            f"BeachAgent.stream called with query: '{query}', sessionId: '{sessionId}'" # Updated log
        )
        if not isinstance(sessionId, str) or not sessionId:
        ...
        if not self.mcp_tools:
            logger.error("No MCP tools available for BeachAgent.stream.") # Updated log
            yield {
                "is_task_complete": True,
                "require_user_input": False,
                "content": "The Beach search tool is currently unavailable for streaming.", # Updated message
            }
            return

        logger.debug(
            f"Using preloaded MCP Tools for Beach stream: {len(self.mcp_tools)} tools." # Updated log
        )
        ...
        logger.debug(
            f"Streaming from Beach agent with input: {langgraph_input} and config: {config}" # Updated log
        )
        try:
            ...
            logger.error(
                f"Error during BeachAgent.stream for session {sessionId}: {e}", # Updated log
                exc_info=True,
            )
            yield {
